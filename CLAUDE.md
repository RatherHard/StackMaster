...
- **三件事必须写清(2026-09-18 下午;均属未落地口径)**:
  ...
  2. **替代形态(页面分发)的设计尚未撰写** —— `docs/develop/前端重设计与分发形态改版.md` **§五** 现为「P0 缺口,尚未撰写」(架构 / 分发载体 / 与 API 的同源关系 / 宿主与用户迁移路径全为空)⇒ **§五 是当前最大阻塞、P0 前置**:在其成文前,计划书与四条底线第 4 条的修订**没有依据可写**,实现亦无从开工。
