...
- **两处冻结契约面变更须先走契约变更流程(尚未启动)**:① **`EMBED_THEMES` 三值**随 `light` / `dark` 退役;② **布局快照面** `columns[].widthRatio` / `columns[].rowHeights` / `viewportWidth` 语义变更。⇒ 顺序 = 改分类论证 → 改契约包与 fixture(`fixtures:manifest` 重算)→ 评审 → 再改实现(契约纪律 5.6);**不得先改实现**。
...
- **遗留承接 = #38**(P0:需求文档与磁盘实现不一致 ⇒ 不处置会让「按文档核对现状」得出错误结论):见 `docs/phases/中期遗留清单.md` **§十一** 与 `docs/phases/中期验收评审.md` **§六 #38**(编号双射);同批 #33 ~ #37 已按改版重估(其中 **#36** 转为「载体变更(不结案)」 —— 免折行实测约束仍有效,改版后约束**左半侧宽度**,**不得当作已解决**)。
