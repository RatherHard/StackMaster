/**
 * 优雅停机协调器(D-MP-5 分支 A;verifier `shutdown.ts` 同款形态,
 * D-API-9 载体纪律延伸)。
 *
 * 停机序列(步骤按注册顺序执行,index.ts / runtime.ts 装配):
 *   close-admin-server(停止接受新连接)→ close-postgres → flush-logs;
 * 超时强制退出码 1;重复信号不重入。
 *
 * Windows 触发通道:无 POSIX 信号投递(process.kill 等价 TerminateProcess),
 * 并行接受 IPC 通道的 `shutdown` 消息触发同一序列(session-api / verifier 同款)。
 */
