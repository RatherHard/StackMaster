 ...
 *
 * 选择器锚 = 结构化属性与 role(表单 input[name]、菜单窗口入口
 * data-window-type / class、语义化 role),不依赖文案排版。
 */
import {
  expect,
  focusWindowButton,
...
test.describe("解题模式档最小集(compose 全拓扑 + plugin-dev 壳)", () => {
  ...
  test("投影渲染(寄存器视图):公开投影寄存器行呈现(非虚拟化路径)", async ({
    ...
    const page = createdSession;
    await focusWindowButton(page, "registers").click();

    // 寄存器窗口:纵向全量白名单寄存器行(M14 不留占位);初始投影
  ...
  test("投影渲染(栈视图):sm-byte-view 字节行出现(虚拟列表;验收锚,登记缺陷)", async ({
    ...
    // 窗口常驻:聚焦即滚动进入视口,初始投影 visibleRegions 直读 → 区域选择器呈现。
    // 窗口集常驻 ⇒ stack / free 两个字节窗口同时在场,字节视图选择器按窗口面板收敛。
    const stackWindow = workspaceWindow(page, "stack");
    await expect(stackWindow.locator("sm-byte-view .region-select")).toBeVisible();

    // 字节行断言(验收锚):虚拟列表出现携带 data-row-address 的数据行。
    // 当前因 vm-ui 虚拟化真实浏览器缺陷红灯(见文件头登记),修复后即绿。
    const rows = stackWindow.locator("sm-byte-view .byte-row[data-row-address]");
