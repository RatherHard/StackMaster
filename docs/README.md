...
| `中期计划.md` | 中期规划(UI 重设计 × 完整可用版本):验收定义 E-1~E-8、终端风格设计语言、Niri 式固定窗口工作区(常驻不关闭、列宽窗高可调)、里程碑 M1~M3 与 WP-70~82、决策点 D-MP-1~7 |
| `contracts/` | **冻结契约与规则文档**(WP 交付物中"活"的规范面):字段分类、DSL 边界、~~嵌入协议~~(2026-09-18 分发改版:**已整体退役**,见下;替代 = **启动票据 / 启动地址**,属新契约面**须先契约后实现**)、版本策略、规范化序列化 |
...
| ~~[嵌入协议.md](contracts/嵌入协议.md)~~ | WP-5 | ~~postMessage 消息契约、handshake 时序与校验规则 V-1 – V-13~~ —— **2026-09-18 退役(分发改版)**:嵌入协议面(`EMBED_PROTOCOL_VERSION` / `EMBED_THEMES` / V-1~V-13)随插件形态**整体退役,不再作为活契约**;替代 = **启动票据 / 启动地址**(属**新契约面,须先契约后实现**)。**本行与文件均不删除**(退役面 ≠ 现在可删;物理下线与实现同批,计划书 §8.2 / §5.6 已改写) |
...
| [中期任务分解.md](phases/中期任务分解.md) | 中期计划(UI 重设计 × 完整可用版本)的执行层拆解:WP-70 ~ WP-82 逐包子任务(文件级落点 + 交付物 + 完成标准)与收口;E-1~E-8 勾稽表、依赖与并行轨 / 关键路径、决策点 D-MP-1~7 执行口径与新识别开口项 O-MP-1~6、门禁与命令清单、逐 WP 红线自查表、文档回填义务清单、风险应对与登记纪律 |
| [中期M2核查表.md](phases/中期M2核查表.md) | 中期 M2 的 **E-4 证据表**:对照 `docs/develop/前端的交互和开发设计.md`(83 行)逐条核查工作区 / 内存视图窗口组件 / 栈视图 / 指令视图 / 寄存器视图 / 自由视图 / payload 搭建窗口,每条给出「符合 / 已定案 / 偏差已双向回填」三态与实现落点或 i18n 键面证据;含 §8 M2 增量一览与 §9 遗留(真机复验阻塞 / WP-74 效果面 / chip tooltip 微瑕 / WP-80 承接项) |
| [中期验收评审.md](phases/中期验收评审.md) | **中期 M1~M3 的收口验收评审**:E-1~E-8 退出条件逐条「达成 / 部分达成(附遗留登记)」+ 可复跑证据、WP-70~WP-83 交付清单、门禁证据汇总(TS / Rust 双 profile / IT / E2E 三浏览器 / 覆盖率 / 契约 fixture / 隔离扫描)、决策点 D-MP-1~7 与 O-MP-1~6 结账、**遗留移交清单**(逐项「仍开放 / 触发条件 / 归属后续」,含证据强度项与事故留档) |
| [中期试用报告模板.md](phases/中期试用报告模板.md) | 首轮试用报告模板:按计划书第十五章 13 项观察项组织,每项标注数据来源(已有指标族 / 受控查询 / 人工访谈)与判据;与 D-API-107 每题观察点对齐(8 题 × 3 项) |
| [中期遗留清单.md](phases/中期遗留清单.md) | **中期遗留项的执行视图(派单与跟踪用)**:32 项按 P0 高优先 / P1 需 CI 清偿与外部阻塞 / P2 证据强度与门禁盲区 / P3 契约与覆盖缺口 / P4 低优先 分组,逐项给「状态 / 归属 / 触发条件 / 下一步动作 / `原§六#N` 映射」,附统计总表、已闭环留档区、环境事实、编号双射一致性要求与**建议派单分组**;与 `中期验收评审.md` §六(评审期登记,承载判定依据与证据原委)**双向约束、须同批更新** |
| [分发改版与UI重设计任务分解与接手入口.md](phases/分发改版与UI重设计任务分解与接手入口.md) | **➡ 分发改版 + UI 重设计的接手入口与排期**(2026-09-18 建立,**待开工**):30 秒现状、必读文档索引、**铁律**(先契约后实现 / 可读性纪律 / 宿主三义项 / 退役≠可删 / 双射同批)、**WP-90~99 排期与关键路径**、门禁命令与已知环境事实、**陷阱清单**(PowerShell 往返 / Vite watcher / 假绿 / 正则误报)、第一批动作、**不要做什么**、已完成清单与交接自检清单 |

### user/(使用文档,面向最终用户)

| 文档 | 面向 | 说明 |
|---|---|---|
| [学习者上手指南.md](user/学习者上手指南.md) | 学习者 | 工作区(固定窗口集 + 列式条带)导览、读内存(窗口 / 锚点 / 跳转链)、Payload 积木、检查点与时间线、调试模式 what-if 纪律、提交与正式裁决(11 值)、16 错误码速查、MVP 八题学习路径 |
| [界面帮助手册.html](user/界面帮助手册.html) | 学习者(界面参考) | 单文件网页手册:页面解剖与工作区布局模型(固定窗口集 / Niri 式列条带 / P0-P1-P2 预设 / 列宽窗高 / 降级渲染)、菜单·状态条·横幅逐项、字节视图(窗口/锚点/跳转/检索/交叉标注/跳转链)、寄存器与教学组件五件套、Payload 三区与积木全表、调试模式与指令视图、提交三态、断线重连、主题三预设(含 terminal 实际观感)、完整交互链(调试→解题→提交→反馈逐步含所见)、动作↔入口对照与窗口语义速查 |
...
| [出题人文档/index.html](user/出题人文档/index.html) | 出题人(分层分页文档,共 11 页) | 目录首页 + 四层:<b>第 0 层</b>认知入门(全局地图 + 31 术语卡)、<b>第 1 层</b>概念详解 7 页(32 个概念统一"一句话/展开/为什么/例子/常见误解"讲透:双包/位宽端序/栈帧劫持/IR 与字节模式/判题时机/谓词/隐藏测试/11 值/stages/种子/ASLR/装载管线/规范化 JSON/登记签名/出题者积木)、<b>第 2 层</b>任务指南(最小题/ret2win/接口 flag 题/排查手册/发布,5 个照着做)、<b>第 3 层</b>精确参考(公开/私有包字段表、20 opcode、谓词/微算子/效果原语、XS+XC 规则速查、错误码/版本/检查清单) |
| [宿主平台接入指南.md](user/宿主平台接入指南.md) | 宿主平台 | ~~架构分工(嵌入协议只承载控制面)、插件文档页部署、embed token 与引导配置端点(通道 a / b)、embed-runtime SDK 用法、宿主校验义务速查(V-1~V-13 人话版)~~、~~能力与外观(terminal 承载口径 D-MP-2)~~、成绩边界 —— **2026-09-18 分发改版注**:上列**嵌入协议 / embed token / embed-runtime SDK / 插件文档页部署 / V-1~V-13 义务 / D-MP-2 承载口径**均随嵌入协议面**整体退役**,替代 = **启动票据 / 启动地址**(平台后端调签发端点 + 下发地址);**成绩边界保留**(`/host/scores`,WP-78,**不属嵌入协议**)。**本行与文件不删除** —— `docs/user/**` 描述**已发布行为**,须随实现落地同批回填(**改版未落地前不改写**) |
| [试用环境部署指南.md](user/试用环境部署指南.md) | 运维 / 部署者 | 试用环境(compose 单机公网)部署与回滚:运维端点暴露面定案(D-API-104 选项② 反代准入)、TLS 终止与域名、最小授权角色连线、环境变量与秘密注入方式(不存秘密、不入 git)、公网暴露面盘点;**纪律 = 未实测不给配置片段** |
...
**2026-09-18 分发改版注(唯一例外面)**:`contracts/嵌入协议.md` **不适用**上述版本演进流程 —— 嵌入协议面已**整体退役**(不做「收窄枚举 + 递增版本」,走硬切 + 迁移指引),替代 = **启动票据 / 启动地址**(属**新契约面,须先契约后实现**,其本身**照常适用**本流程)。退役面的**物理下线与代码删除不在本批**(须与实现同批;计划书 §8.2 / §5.6 与 `CLAUDE.md`「在途改版」已记此纪律)。
