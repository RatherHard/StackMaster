...
| [界面帮助手册.html](user/界面帮助手册.html) | 学习者(界面参考) | 单文件网页手册:页面解剖与列式布局模型、菜单/状态条/横幅逐项、字节视图(窗口/锚点/跳转/检索/交叉标注/跳转链)、寄存器与教学组件五件套、Payload 三区与积木全表、调试模式与指令视图、提交三态、断线重连、完整交互链(调试→解题→提交→反馈逐步含所见)、动作↔入口对照与窗口语义速查 |
