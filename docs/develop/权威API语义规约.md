...
- **2026-09-18 补记:主控现场定案(四项 + 一处口径订正)**:
  1. **嵌入协议面整体退役** —— **取消插件形式,页面分发为唯一形态**。⇒ `EMBED_PROTOCOL_VERSION` / `EMBED_THEMES` / 嵌入协议 V-1~V-13 / `packages/embed-runtime` / `packages/react-wrapper` / `packages/web-component` **随协议整体退役,不做版本演进**。**本条取代**本条目「决策 4」中「`EMBED_THEMES` 须走契约变更流程」的表述 —— **不是「收窄枚举 + 递增版本」,而是随协议退役**(该路径作废)。**本项属底线级 + 产品定位级**变更 ⇒ 须修订 `CLAUDE.md` **四条底线第 4 条**与 `docs/项目计划书.md`(§一 / §二 产品定位、§5.4 与 `:341` 分发形态、第八章 插件嵌入与协议、5.5 结构);**且替代形态(页面分发)的设计尚未撰写** ⇒ **在计划书修订与替代设计落地前,磁盘实现与唯一权威来源仍按现状有效;不得据本项删代码、删契约或改 E2E 断言。**
  ...
  - **仍未定案(不得默认忽略)**:左右两分比例可调性;可见视图位数量;`Ctrl+↑/↓` 快捷键冲突;拖拽落点语义;窄屏降级;主题 token 收敛口径;列表的键盘等价路径;**分发形态设计本身(需求文档 §五 仍为留白 —— 架构 / 分发载体 / 与 API 同源关系 / 迁移路径,原 §四 Q1 / Q3 / Q4 / Q5 均未答)**。
