...
## 三·二十一、中期 M1「可用性地基」(中期 WP-70 ~ WP-73;D-API-110 ~ 116)
...
- **遗留(如实登记)**:①跨源宿主无法直接写插件文档内的锚(同源策略),承载面 = 插件文档页 / 部署配置;若需宿主侧可达的配置面(如 `PWN_MEMORY_VM_CONFIG` 的 additive 主题项),属 WP-74 / WP-77 决策点,本包未越界新增公开配置面。②jsdom 不跨 shadow 边界传播自定义属性的计算值(实测),组件 shadow 内的主题计算值机检与真机 axe 校准归 WP-74。③扫描线 / 光标闪烁的**实装**与 `aria-hidden` / `prefers-reduced-motion` / 字号下限 13px 归 WP-74;本包只落效果类变量且 light / dark 下取值恒为关闭(`--sm-scanline-opacity: 0` / `--sm-caret-blink: 0s`)。**复核(2026-09-17,主控):②③ 已由 WP-74 达成** —— ②的「组件 shadow 内主题计算值机检」已升级为**自校验全组件机检**(消费面 + 豁免面的并集必须等于源码 `@customElement` 真实清单,另含字号下限 rem×16 折算与字体栈回退值漂移检查),真机 axe 校准已扩为**九面 `violations = 0`**(归档 `e2e/reports/axe/2026-09-17/`,口径见 **D-API-121**);③效果面(扫描线 / 光标闪烁 / 终端标题栏)已实装并真机验证,`aria-hidden` + 零文本 + 零可聚焦后代、`@media (prefers-reduced-motion)` 降级、13px 下限均有断言。①(宿主侧可达配置面)仍为「本包未越界新增」的决议项,维持原状。
...
- **定案**:
  ...
  5. **视口外降级渲染择一**:面板 `content-visibility: auto` + `contain-intrinsic-size: auto 9rem` + `data-render-degrade` 标记。判据:零 JS、零浮动层、零新增依赖,浏览器原生跳过离屏子树的渲染与绘制(重点覆盖 `payload`(Blockly 挂载即 inject)与 `debug` 指令视图)。**该占位值已于 2026-09-17 由 D-API-152 取代:改为 `MIN_ROW_HEIGHT_PX` = 266px(原 `9rem` 是无依据的裸值);本条其余判据不变。**
...
### D-API-116 无障碍语义面收敛(固定窗口集暴露的 axe 违规清零;WP-74 前置)

- **触发 WP**:WP-74 前置修复(M1 的 WP-71 使 10 个窗口**常驻 DOM**,此前互不同时在场的内部视图进入同一次 axe 扫描)。**判定依据(可复跑)**:逐面比对 HEAD 归档 `git show HEAD:apps/plugin-dev/e2e/reports/axe/2026-09-11/*.json` ⇒ 五面 `violations = 0`,且 `plugin-iframe-dark-registers` 的 `color-contrast` 仅检查 **1** 个节点、全文零 `blockly` 命中 ⇒ 相关节点在 M1 前**根本不在场**;WP-72 的归因实验(关闭 `content-visibility` 降级渲染复跑同例仍红)排除降级渲染嫌疑。
- **定案 1(日志 live region)**:`<sm-payload-tab>` 执行日志的 live region 语义由 `aria-live="polite"` 单独承载,**不在 `<ol>` 上声明 `role="log"`** —— role=log 对 `<ol>` 非法(`aria-allowed-role`),且覆盖其隐式 list 角色使 4 条 `<li>` 逐行失去 list 父级(`listitem` serious);即现状语义本身是坏的,修正后列表语义与播报机制双双成立。**否决**:外层 `<div role="log">` 包裹 —— 输出区 `<ol>` 自身即 flex 滚动容器(`.pane ol { flex:1; overflow:auto }`),引入包裹层必须连带改 CSS 几何,违背「语义-only / 视觉零变化」;把 `<li>` 改 `<div>`(丢列表语义)。
- **定案 2(地标命名;`landmark-unique`)**:窗口面板 `aria-label` **仍逐字等于注册表展示名**(WP-71「标题栏与 aria-label 同源」承诺与其断言零修改即绿);改为**内层 region 名带窗口维度**:`byte.viewAria = "{view} · 字节区"`、`instr.viewAria = "{view} · 指令流"`(`{view}` 取注册表 `labelKey`,与窗口标题同源)、`vma.ariaScoped = "{view} · VMA 列表"`(`SmVmaList` 新增语义维度属性 `viewLabel`,由宿主 `byte-tab.ts` 按窗口类型注入;独立使用形态回落 `vma.aria`)。两个常驻字节窗口的 VMA 列表由此按窗口维度分离。`instr.aria` **退役**(与 `tab.instruction` 同文案 = 双源漂移)。**否决**:面板改 `aria-labelledby`(名称字符串不变、规则不消,且砍掉被 E2E 消费的稳定锚)、取消内层 region 角色(减语义,VMA 侧栏有独立部件语义不能删)、面板名加「窗口」前缀(面更大且触碰 WP-71 承诺面)、VMA 列表去名(删地标)。
- **定案 3(Blockly 画布色源)**:Blockly 画布经**官方主题 API**(`Blockly.Theme.defineTheme` 的 `componentStyles`)消费既有主题 token,映射 6 项:`workspaceBackgroundColour` → `--sm-bg-base`、`toolboxBackgroundColour` → `--sm-bg-panel`、`toolboxForegroundColour` → `--sm-fg`、`flyoutBackgroundColour` → `--sm-bg-inset`、`flyoutForegroundColour` → `--sm-fg`、`scrollbarColour` → `--sm-fg-dim`;值一律 `var(--sm-*)`,**组件内零色值复制**(内联 + `var()` ⇒ light / dark / terminal 三档换档即时生效,无需重新 inject)。**未映射** `cursorColour` / `markerColour` / `insertionMarkerColour` 与各 `*Opacity`:它们经 `getComponentStyle` 读入**渲染器常量**并写成 SVG **属性**,属性不解析 `var()`,映射会静默失效。**唯一必需的 CSS 改写** = 同根注入样式表(`sm-payload-canvas-style`,幂等,作用域 `.payload-canvas-host`)中的 `.blocklyToolboxCategoryLabel { color: var(--sm-fg) }` —— Blockly 自带 CSS 的 `color:#fff` 是**标签自身声明**,继承不可覆盖;另有 `.blocklySvg` 背景(网格开启时 `workspaceBackgroundColour` 走网格图案路径、不落 `.blocklySvg`,而 Blockly 自带 `background-color:#fff` 是字面量)。**刻意视觉变更(登记)**:暗色与 terminal 档下 Blockly 工具箱 / 画布底 / 飞出由默认浅色(#dddddd / #fff)改为主题深色;亮色档按 token 解析为近白面板与黑字(实测对比度 18.87:1,暗色 17.66:1,修复前 1.35:1)。**否决**:走 Blockly「白字 + 类目配色底」原生设计(白字对中亮度类目底仍可能 <4.5:1,把确定性违规换成碰运气)、全量自写 `.blockly*` 覆盖(与上游样式表强耦合)、仅祖先挂内联 `color`(实测无效)、保留浅色画布(不满足 token 化方向)、`filter: invert(1)` 修深底精灵(**机制不可达**:样式表在画布宿主 shadow 根内而主题锚在树外,树内样式表不能匹配树外祖先 —— 实测写入后 `getComputedStyle(.blocklyTrash).filter === "none"`,故该无效规则已删除,不留死代码)。
...
- **遗留(原「移交 WP-74」,2026-09-17 复核后重新归属:M2 关闭时 ① 未被承接 ⇒ 移交 M3)**:①深色画布上**垃圾桶 / 缩放光栅精灵**的可见性(修法 = 把主题锚镜像进 shadow 树,或新增「深底精灵处理」token,**须先定 token 名与值**;承接载体 = **M3 WP-80**(**2026-09-17 订正**:原写「M3 WP-81 遗留移交清单」是失效指针 —— WP-81 实为 canary 契约收口〔`docs/phases/中期任务分解.md:342`〕;按 M3 分派口径**丙**逐项核对后落 WP-80 = M3 中唯一触及 Blockly 画布与 `blockly-theme.ts` 主题接缝者)**—— WP-74 已于 2026-09-17 复核确认未承接此项,原「归 WP-74」指针在 M2 关闭后失效,故正式移交 M3**;②`color-contrast: incomplete ×7`(Blockly 类目标签 `bgOverlap`)沿 WP-55 口径属人工复核项(修复前 light 面即为 ×7),建议不新增豁免(**复核确认:2026-09-17 九面归档中 iframe 六面仍各 ×7,与修复前一致,属沿袭的人工复核项,非 M2 引入**);③~~`terminal` 预设未进真机 axe 面~~ **已于 WP-74 达成**(九面含 `plugin-iframe-terminal-{registers,stack}`,`checked=7` 各面 `violations=0`);④`shell dark` 面沿既有登记不可达(复核确认:`degraded terminal` 亦不可达,理由见 D-API-121)。
...
- **定案 1(零新增 token,硬约束)**:`SM_THEME_VARIABLES` 的 **20 枚**名清单是硬约束 —— `test/theming/theme-terminal.test.ts:159` 有**精确键集断言**,且「20 token × 3 预设」已作为既成事实登记于 `CLAUDE.md` 与 `packages/vm-ui/README.md` ⇒ **加 token 必然打红三处**。WP-74 因此只做**消费**。无法用现有 token 表达的系统色(`mark` / `highlighttext` 等)一律**逐字保留**,并登记为「**待 axe 真机裁决**的候选 token 表」,由主控按真机 color-contrast 结果决定是否立项(先量后定,不先改 token 面)。**⚠ 2026-09-17 M3 收口订正:本定案的「零新增 token」绝对表述已被 D-API-130 取代(非废止其纪律,而是补上了本定案预留的例外通道)**:`--sm-canvas-sprite-filter` 作为**第 21 枚** token 由 **WP-80** 落地(授权来源 = 本定案自身预留的「候选 token 表 —— 待 axe 真机裁决、由主控决定是否立项」通道:M1 遗留项「深色画布垃圾桶 / 缩放光栅精灵可见性」经 axe 真机取证后由主控派单 M3 WP-80)。⇒ 阅读本条时的正确口径:**「先量后定、不先改 token 面」的纪律仍然有效**;「20 枚」是 **M1/M2 时点事实**,**当前为 21 枚**(三处登记 `CLAUDE.md` / `packages/vm-ui/README.md` / `theme-terminal.test.ts` 均已同步为 21)。本注为**追加**,其上原文逐字保留以免抹去历史口径。
...
- **缺陷 2(题目形态面;已修,commit `c295c87`)**:缺陷 1 接线后,真机 `debug_attach` 收到的**不是** `debug_attached`,而是 `{"type":"error","payload":{"code":"internal_error","message":"internal error"}}`;session-api 日志 `component=debug-channel … type="DebugVariantBuildError" … msg="debug frame failed with unknown error"` ⇒ **调试实例变体构建失败** ⇒ 无 attach 回执、无 `debug_pausedAddressHex`、指令流为空(`instructions().length === 0`),前端指令视图落为「调试实例:尚未 attach」空态。**判定**:与缺陷 1 是**两个独立故障**(路由是否挂载 vs 变体能否构建)。
- **缺陷 2 根因(本条的核心价值,已于 2026-09-17 定位)**:抛出点 = `packages/challenge-compiler/src/debug-variant/build-debug-variant.ts:238`(`:232` 判据),规则 `XC-DEBUG-MODE-IR`;真机调用链 = `runtime.ts:398` 的 `productionDebugVariantProvider` → `debug-variant-provider.ts:124` → 抛点 → 通道兜底(`debug-channel.ts` 原 `:380-383`)只登记 `type` ⇒ 规则 ID 丢失、真机日志只剩「unknown error」。**根因不在通道实现,而在题目形态**:`apps/session-api/k6/seed-challenge.mjs` 登记的演示题是 **IR 模式**(`compiledIr`、无 `encodingTable`、`RIP = "0x0"`),而调试变体契约**不携带程序 IR / 入口声明面**(ADR-DC1 条款 2:调试实例**恒字节模式**装配)⇒ 该题目**结构性**无法产出调试变体。**影响面**:该脚本正是 E2E `seedChallenge` 与手工演示拓扑**唯一驱动**的题目,且公开描述包 `debugMode` 是 **opt-out 缺省 = true** ⇒ 浏览器侧必然出现一个**永远点不通的调试开关**。
- **缺陷 2 漏网根因(与缺陷 1 同族,第三次现形)**:Node 侧测试要么走 rig 的**占位变体供给**(`placeholderDebugVariantProvider`),要么在 `debug-variant-provider.test.ts` 里把 **IR 拒绝当作期望行为断言** ⇒ 无一条覆盖「演示 / E2E 真实登记题目的形态 + 生产 Provider」。**修法**:种子改**字节模式**(`entrypointAddressHex: "0x401000"` + `vmProfile.encodingTable` 覆盖代码区全部字节:入口 `0xc3` = `ret`、其余 `0x00` = `mov RAX,RAX`;装载期译码探测 `XS-ENC-PROBE` 因此不红;去掉 `compiledIr`),私有 `initialState.registers.RIP` 与公开 `initialProjection.visibleRegisters.RIP` **同步对齐 `0x401000`**(XS-PROJ-VALUES 镜像义务;**RIP 必须对齐**:调试实例指令指针 = 变体 `registers.RIP`,而指令流只在**暂停时**按暂停落点下发 ⇒ RIP 落区域外时暂停落点无覆盖、指令视图恒空);通道兜底日志补 `message` + `violations[].ruleId`(**呈现面一字未动**:wire 帧恒冻结 `internal_error`,与 `productionDebugVariantProvider` 的映射定案一致;违规 message / path 不入日志,可能内嵌私有引用 ID)。**绿灯**:新增 `test/debug/demo-challenge-debug-capability.test.ts`(走**生产变体供给路径**)修前 **4 红**、修后 **4 绿**(IT 门控 +1 例走**真实 Rust worker**,attach 成功且暂停落点有指令覆盖 `text = "ret"`);`debug-channel.test.ts` 新增日志归因用例修前红 / 修后绿;真机 IT `test/debug` + `test/scan` **99 passed**。**未触碰任何冻结面**(协议帧集 / `PublicError` 码 / `protocol` / `challenge-schema` / `vm-engine` / §九 推送模型零改动)。
- **缺陷 2 的纪律启示(补 D-API-120 待办)**:缺陷 1 的待办是「装配完整性机检」;缺陷 2 补一条同类义务 —— **凡消费「题目形态」的通道,其测试必须至少有一条覆盖「生产种子 / 真实登记题目的形态」**(而非只覆盖占位供给或把拒绝当期望),否则契约与题目形态的不相容永远只能在真机上暴露。

### D-API-121 axe 真机扩面的判定口径、不可达格与首跑缺陷(WP-74)

- **触发 WP**:WP-74(axe 真机门禁从「chromium × 6 面」扩为「三预设 × 9 面」,并新增 `e2e-matrix` job)。
- **判定口径(定案)**:每扫描面 **`violations === []` 且 `colorContrastStatus !== "absent"`**(规则必须真实执行);判定桶 `passed` / `incomplete` 均为已执行,`incomplete` = 引擎无法自动判定 ⇒ **人工复核项**,不新增豁免(沿 WP-55 口径)。归档路径按**运行当日** `e2e/reports/axe/<YYYY-MM-DD>/`,**无覆盖开关** —— 归档即历史证据,历史目录只增不改(WP-55 的 `2026-09-11/` 不再被覆写)。
- **三处不可达格(如实登记,不制造假绿)**:①`shell dark` 与 ②`degraded dark`(沿既有登记:本机 headless 不绘制 root 级暗色画布 = 半暗态伪影;降级形态无握手 ⇒ 无 `theme_changed` 通道);③**`degraded terminal`(本包新增登记)** —— 降级形态**不安装文档级主题 token 样式表**(该表由组件侧 `ensureSmThemeStyles` 安装,而降级形态无 `sm-workspace`、无 themed 组件树)⇒ 锚 attribute 在场且被保留,但 `--sm-bg-base` 的 computed 值为**空串**,变量无处级联。**处置**:只断言锚写回的**结构事实**、不扫描该格;**明确拒绝用 harness 注入产品路径不会产生的样式表**来制造可达性。**一般性口径**:不可达格必须写明「为什么结构上不可达」,且不得用测试侧注入掩盖。
- **首跑抓出的真缺陷(axe 价值证明)**:`plugin-iframe-dark-registers` 面 `.pseudo-asm[data-pseudo-asm-source="solve"]`(与 `no-coverage`)前景 `--sm-fg-dim`(`graytext` = `#808080`)落在 `--sm-bg-panel`(暗档 `#131921`)上 = **4.47:1**,低于正文 4.5:1(serious,13px)。**该节点此前不可达**:跳转链死绑定(D-API-117)修好前链恒空 ⇒ M1 的六面归档里此节点**根本不在场**(与 M1「暗色 Blockly」1.35:1 的判定同机理)。**修法**:降级形态**不再降前景色**、取正文前景 `--sm-fg`;「降级 vs 真指令」的区分改由**系统字体 + 「(暂无伪汇编数据)」标题**承担。**零新增 token**。
...
### D-API-152 工作区窗高语义:下限按可读目标推导 + 溢出列拖拽的像素语义(用户验收测试缺陷订正)

- **决策**(五点定案,**取代**原裸 `9rem` = 144px 的下限):
  1. **高度下限按可读目标推导**:`MIN_ROW_HEIGHT_PX = ceil(面板 chrome 182.1px + N(=4) × 行高 20.8px) = ` **266px**(`packages/vm-ui/src/workspace/layout-divider.ts:53`,由常量插值、无魔数)。推导输入(chrome 逐项 = 标题栏 26 + 字节视图边框 1 + 工具区 133.3 + 列头行 21.8;行单位 20.8 = 13px 字号 × 1.6 行高)与取值理由登记在 `layout-presets.ts` 的「块轴(高度)阈值推导」段,与宽度侧(`58ch × 7.8px = 452.4px`)**同法**。**N = 4 的理由 = MVP 栈帧教学闭环(4 行 = 一次 `read` 的 32 字节跨度)**。
  2. **列高下限是比例的函数,不是窗口数的函数**:`columnMinHeightPx(heights) = max_i(MIN_ROW_HEIGHT_PX / ratio_i) + columnChromePx(n)`。等分时**逐值退化**为原算式(1/2/3/4/10 ⇒ 268/560/852/1144/2896),故属**推广**而非推翻;比例偏斜时列盒更高。
  3. **像素换算基准 = 自由空间**,唯一化为 `F = 列高 − columnChromePx(n)`,其中 `columnChromePx(n) = 2n 面板边框 + (n−1) 分隔条 + (2n−2) 间距`(旧实现拿列高当基准,852/28 时偏差 3.3%)。
  4. **溢出列的拖拽语义**:`rowHeightsAfterDrag` 改**像素语义** —— `pixels_i = ratio_i × F`、`nextBefore = max(before + Δ, MIN_ROW_HEIGHT_PX)`、`nextAfter = max(after − Δ, MIN_ROW_HEIGHT_PX)`,比例 = 新像素 ÷ 新像素和。**富余列(两窗都在下限之上)⇒ 像素和守恒,既有不变量不变;被压窗贴住下限后,多余位移转为列盒长高**(列盒 ≡ 内容高,由滚动承载),**被压窗永不低于下限**。
  5. **拖拽像素基准在 `pointerdown` 冻结**(`startColumnHeightPx`):旧实现每步重读 `clientHeight`,而列高在拖拽中会被下限顶高 ⇒ 同一份 `startHeights` 按新自由空间重新摊开 ⇒ 非相邻窗跟着变高、列盒长过位移(真机实测 **+60px 顶高列盒 137px**)。冻结后终态 = `(startHeights, 冻结基准, Δ)` 的**纯函数**,分隔条严格跟随指针。
- **理由**:(1) 宽度侧下限严格推导、高度侧却是**无依据的裸值**,这条不对称叠加「固定窗口集(10 类常驻、窗口数不随视口变化)」⇒ 窗口数多的列必然被压到不可读;**且因下限存在(并非缺失),它一路躲过了 axe 9/9 面与全套 E2E** —— 「下限有无依据」比「有无护栏」隐蔽得多。真机取证(中宽 P1、嵌入形态、3 列):列高 656px,3 窗列各 202.7px、4 窗列各 **146px**(贴着 144 + 上下边框),而面板 chrome 实测 **182.1px** ⇒ **144px 的下限连一行字节都装不下**。(2) **比例是权威**:用户面表述「上下两窗**比例**之和恒定」在两种情形下都为真;像素和只在富余列守恒,溢出列本就**应该**增长(否则「增大」在数学上不可能)。(3) 冻结基准把「同一手势的终态」从依赖拖拽途中几何改为纯函数,是确定性要求。
- **红灯位置**:`test/workspace/layout-presets.test.ts`(+5:行单位 / 列头行 / chrome 逐项 / N=4 / 下限 = `ceil(chrome + N × 行单位)` = 266);`test/workspace/layout-divider.test.ts`(**18 例**:下限推导式;像素语义;被压窗贴底 + 像素和增长;富余列守恒;列盒随比例偏斜;逐例口径机检 `ratio_i × F ≥ 266`;溢出列两条 regime);`test/workspace/sm-workspace-layout.test.ts`(**40 例**:列高下限随比例;拖拽基准冻结 —— 按下时 H=560、拖拽途中把 mock `clientHeight` 改成 900,终态仍按 560);`test/render/pre-whitespace-family.test.ts`(**9 例**,见影响面 a);**真机**:`apps/plugin-dev/e2e/workspace-layout.spec.ts` **9 例**(原 8 例**逐字未改、断言零放宽**;新增第 9 例 = 溢出列 regime:被压窗 `≥ MIN_ROW_HEIGHT_PX`、非相邻窗不动、列盒增量 ≡ 被增大窗增量)。
- **影响面与遗留(如实登记)**:
  (a) **伴生缺陷(同批修复)**:字节视图「特殊显示」列的 Lit 模板把**字面换行写在声明了 `white-space: pre` 的标签内部**(`src/render/special-display.ts` × `src/views/byte/byte-view.ts:744-747`)⇒ 每个 cell 渲染 2 个行盒 ⇒ 数据行实测 **187.17px**(应为 20.8px)。**该缺陷使 ① 的下限一度形同虚设**(266 − 182.1 = 83.9px 装不下一行 187px)。**真机修后**:普通数据行 **20.8px ✔**;挂跳转链的行 93.19px(= 20.8 + 72.39 跳转链块)、挂寄存器标注的行 43.59px —— **两者余高归因到别的组件,非本族产物**。同族第二实例 = `src/views/instruction/sm-instruction-view.ts:681` 的 `.jump-target` 按钮(继承 `.row-text` 的 `pre-wrap`),一并修复。**机检** = `test/render/pre-whitespace-family.test.ts`(源码文本扫描 + CSS 继承语义 + 跨文件作用域「双向一跳」;修复前临时回退实测 **3 条真实违规必红**:两个 span + 该 button,含跨文件声明归因与继承归因)。覆盖边界(跨组件 shadow 继承 / flex 空白折叠 / 动态类名不建模)已在文件头登记。
  (b) **诊断订正(留档,勿沿用错判)**:主控曾判定「`flex-basis: 0` + 比例和恒 1 ⇒ 自由空间恒为 0 ⇒ 拖拽数学上不可能」——**被真机实测证伪**:1440×900 下列 0 `clientHeight 852`、两窗 414/414、自由空间 824。**真实成因** = 266px 下限把 3 窗列顶到 852px ⇒ 工作区 1163.3px、页面高 **1322px** ⇒ 交互几何落到 **900px 视口之外**,浏览器对**视口外坐标不命中**页面内容(`event.target === html`)⇒ 组件挂点收不到事件(逐事件 `doc:` / `shadow:` 双挂点对照取证)。`:199` 的落点需要 `条带顶 < 399px`,而实测菜单 + 简介 + 状态行 = **294px**(条带顶 453)⇒ **产品侧无解**(除非放宽 266px 下限或废掉 `align-items: stretch` 的列弹性;旧下限 144px 时该点只剩 **11px** 余量 ⇒ 旧用例是靠这 11px 侥幸绿的)。**修法落在 E2E 夹具**:拖拽前把**起点与终点一起**滚进视口(真实用户同样先滚动),无法同时容纳时**抛错而非静默拖到空气上**;**未改任何断言、未改落点语义**。
  (c) **本次新登记的遗留**:见 `docs/phases/中期遗留清单.md` **#33 ~ #37**(几何 / 可读性无门禁;拖拽落点出视口即不可达;开发壳 `.tab-area` 无高度上限;`MIN_COLUMN_WIDTH` 的「十六进制行不折行」前提在现有三档列宽下不成立;宿主未给工作区定高致条带自身纵向滚动在两种已发布形态下都不会出现)。评审侧 `docs/phases/中期验收评审.md` §六 同步同号登记。
  (d) **未实测(明文登记,不得视为通过)**:`test:coverage` 完整形态、`test:miri`、`fuzz:smoke`、全量 E2E、`E2E_MATRIX`(webkit 12 格)、`test:compose`(本机镜像不可重建)。

### D-API-153 前端改版:页面级整页布局 + 终端单主题(需求层定案;**实现未落地**)

- **决策**(需求层四点定案;主控 2026-09-18 口述要求,逐条钉死笔误与歧义后注入需求文档):
  1. **整页布局**:工作区**占据整个页面**,不再依赖宿主给出的高度 ⇒ 上一版「宿主未定高 ⇒ 条带自身纵向滚动不出现」的**前提问题消解**。
  2. **左右两分**:右半侧 = **payload 搭建窗口(固定)**;左半侧 = **视图管理窗口**,在**上下两半**显示视图(任一时刻**两个可见视图位**),视图**纵向堆叠、上下滚动**,滚动带**丝滑动画**。
  3. **无边框紧密贴合**:视图间**无间隙、无描边、无分隔条**;**视图类型名写在视图内左上角**,不设独立标题栏 ⇒ **窗高下限(D-API-152 的 `MIN_ROW_HEIGHT_PX` = 266px)与列高下限(`columnMinHeightPx` / `columnChromePx` / 拖拽像素语义)一并废止**。
  4. **终端单主题**:UI 风格 = **简洁的黑客 Linux 终端风格**;主题**只保留终端一种**,`light` / `dark` **退役**。
  另附两项交互:**列表按钮**(可展开 / 收起;勾选控制左半侧显示哪些视图;拖拽调整排布顺序)与 **`Ctrl + ↑/↓` 上下切换视图窗口**。
- **「暂离」语义扩展(D-MP-1 不修订)**:取消勾选 ⇒ 视图**不在左半侧显示**,但**实例仍常驻** ⇒ 「暂离」现有**两种成因**(① 滚出可视区;② 未勾选)。**「无开 / 关状态」这一条不变** —— 用户能决定**看什么**,但不能**销毁视图**。
- **可读性纪律(本决策最易被误读的一条)**:废止窗高下限 **≠ 放松可读性** —— 本版把「**压缩以适配**」这一机制**整条移除**,改由「**左半侧纵向滚动 + 每个视图位有确定高度**」满足可读性。**判据**:落地实现若仍出现「视图被压到**装不下一行字节**」,即**违反本决策**。
  - **历史反例(不得重演)**:2026-09-17 取证 —— 4 窗列每窗 **146px**,而面板 chrome 实测 **182.1px** ⇒ 连一行字节都看不见(取证与机制详见 D-API-152「影响面」)。
- **理由**:(1) 上一版把 **10 个常驻窗口**挤进有限视口高度,「压缩以适配」与「可读」**结构性冲突**,只能靠高度下限缓解;**本版换掉机制** —— 溢出交给滚动,冲突消失,下限随之失去存在理由。(2) 把「暂离」由「滚出视野」扩展为「**滚出视野 ∨ 未勾选**」,使**显示控制**与**生命周期**分离:用户能决定看什么,**固定窗口集(D-MP-1)的保证不被削弱**。(3) 终端单主题使视觉面收敛(设计语言唯一),代价是 `EMBED_THEMES` 冻结三值须走契约变更。
- **取代关系**:**取代**列条带模型全部(Niri 式列间水平滚动、列内二叉分割、P0 / P1 / P2 预设与阈值表 `MIN_COLUMN_WIDTH` / `WIDE_MIN_PX` / `NARROW_MAX_PX`、列宽五档、列间与窗间分隔条、三类拖拽落点、重置布局、焦点列居中相机)与 **D-API-152 的几何面**;**保留** `content-visibility` 视口外降级渲染与 `prefers-reduced-motion` 降级(后者降级对象由**相机滚动**改为**丝滑滚动动画**)。
  **D-API-152 条目本身作为历史决策保留在案,不得删除。**
- **实现状态:未落地(须显式声明,勿当现状)** —— 磁盘实现仍是上一版(Niri 式列条带);需求文档 `docs/develop/前端的交互和开发设计.md` 的「工作区组件」一节现为**目标态** ⇒ **现状核对 / E2E 断言 / 用户文档一律以磁盘实现为准**,直到改版落地后统一回填。
- **派生义务(未完成)**:(a) **契约面一** —— `EMBED_THEMES` 三值随 `light` / `dark` 退役 ⇒ 须走契约变更流程(先改分类论证 → 改契约包与 fixture → 评审 → 再改实现)并重算 `fixtures:manifest`;(b) **契约面二** —— 布局快照面 `columns[].widthRatio` / `columns[].rowHeights` / `viewportWidth` 语义变更 ⇒ `packages/protocol` 与冻结 fixture 受影响;(c) **遗留 #38** 承接 (a)(b) 与用户文档回填;(d) 遗留 **#33 ~ #37** 按本版**重估**(见 `docs/phases/中期遗留清单.md` §十一 与 `docs/phases/中期验收评审.md` §六)。
- **红灯位置**:**本决策为需求层定案,无实现、无真机证据** ⇒ 不得据本条目宣称任何功能已达成。改版实施时的红灯面(需求文档「本版待定案」**11 项**):左右比例可调性;可见视图位数量;菜单四组调整(「布局」组已无对应物);`Ctrl + ↑/↓` 与浏览器 / OS 快捷键冲突;拖拽落点语义去留;布局快照面迁移;窄屏降级;主题退役后 token 集合收敛;无边框 + 类型名内移后的无障碍基线(axe 真机 `violations = 0` 不得回退);**「占据整个页面」在嵌入形态下的含义(与分发形态改版耦合 —— 定案前不得宣称「整页布局已解决宿主定高问题」)**;**契约与版本策略(`EMBED_THEMES` 退役后是否递增嵌入协议版本号 / 是否保留 N-1 兼容窗口)**。
- **2026-09-18 补记:主控现场定案(四项 + 一处口径订正)**:
  1. **嵌入协议面整体退役** —— **取消插件形式,页面分发为唯一形态**。⇒ `EMBED_PROTOCOL_VERSION` / `EMBED_THEMES` / 嵌入协议 V-1~V-13 / `packages/embed-runtime` / `packages/react-wrapper` / `packages/web-component` **随协议整体退役,不做版本演进**。**本条取代**本条目「决策 4」中「`EMBED_THEMES` 须走契约变更流程」的表述 —— **不是「收窄枚举 + 递增版本」,而是随协议退役**(该路径作废)。**本项属底线级 + 产品定位级**变更 ⇒ 须修订 `CLAUDE.md` **四条底线第 4 条**与 `docs/项目计划书.md`(§一 / §二 产品定位、§5.4 与 `:341` 分发形态、第八章 插件嵌入与协议、5.5 结构);**替代形态(页面分发)的设计已于 2026-09-18 成文**(见本补记末段「分发形态设计已成文 —— 解除前述当前最大阻塞」)⇒ **在计划书修订与实现落地前,磁盘实现与唯一权威来源仍按现状有效;不得据本项删代码、删契约或改 E2E 断言。**
  2. **「工作区占据整个页面」= 顶层页面 `100dvh` + 左半侧内部滚动**(无宿主高度上报、无 iframe)。原「嵌入形态下到底指什么」**随第 1 项消解**;**`MAX_EMBED_HEIGHT_PX` / `height_changed` / `auto_resize` 属退役面**。
  3. **布局快照面处置 —— 并订正本条目的一处错误**:查证 `WorkspaceLayoutSnapshot` 定义于 `packages/vm-ui/src/workspace/workspace-model.ts:92`,而 **`packages/protocol` 对 `layoutSnapshot` / `widthRatio` / `rowHeights` / `viewportWidth` 零命中** ⇒ **它不是冻结契约面**。**本条目「派生义务 (b) 契约面二」的表述作废(该表述是错的)** ⇒ 按新模型**自由重构字段 + 同步改 vm-ui 测试**;且**全仓 `idb-keyval` / `indexedDB` 零命中** ⇒ 布局**无持久化面,无迁移义务**。
  4. **菜单**:原「布局」组 → **「视图」组**(勾选显示 / 排序 / 「重置视图」= 恢复默认顺序与全选)⇒ 菜单四组 = **窗口 / 视图 / 模式 / 运行**。**⚠ 与左半侧列表按钮功能重叠**,「谁是唯一入口」**仍未定**。
  - **仍未定案(不得默认忽略)**:左右两分比例可调性;可见视图位数量;`Ctrl+↑/↓` 快捷键冲突;拖拽落点语义;窄屏降级;主题 token 收敛口径;列表的键盘等价路径;~~分发形态设计本身(架构 / 分发载体 / 与 API 同源关系 / 迁移路径)~~ —— **已于 2026-09-18 定案成文**(见本补记末段);其**仍存开口** = 票据形态(URL query 还是 fragment;长度 / 熵 / 有效期 / 单次消费与重放防护);**签发端点**的路由 / 鉴权 / 限流键与错误语义(**属新契约面,须先契约后实现**);换票响应与失败语义;票据进访问日志 / Referer 泄漏的处置;**产品定位表述与 `docs/user/**` 回填时机**。
  - **派生义务变更**(据上述):原 (a) 由「契约版本递增」**转为「嵌入协议整体退役」**(含 `docs/contracts/嵌入协议.md` 下线与三包处置);原 (b) **撤销**(非契约面);**新增:计划书 + 四条底线第 4 条的修订 = P0 前置**(权威来源级,先于任何实现)。
  - **分发形态设计已成文(工作稿 §五,2026-09-18 —— 解除前述「当前最大阻塞」)**:四项定案 = ① **载体与同源** = **与 session-api 同源**(页面由 session-api 或**同域反代**提供,同域两路径);② **授权入口** = **服务端下发地址**(服务端签发票据 → 地址携带 → 页面服务端换票;主控原话「**跟 CTF 平台一样服务端下发一个地址**」);③ **题目与租户定位** = **服务端分发、一题一址**,`challengeId` / `version` 属**公开导航信息**,**租户与授权由服务端绑定、不由 URL 自报**(主控原话「**服务端分发,和 CTF 平台一样的流程**」);④ **迁移** = **硬切 + 迁移指引**(**不留并存窗口**;退役走下线公告 + 指引,不适用契约纪律 5.6 的 N-1 演进窗口)。
    - **保留面(勿误伤)**:`/host/scores`(WP-78 宿主成绩同步)**保留** —— 它**不是嵌入协议的一部分**,平台仍需拉成绩;**退役面 ≠ 全部宿主面**;`SESSION_API_HOST_BACKEND_TOKEN` **保留并扩展用途**(签发端点亦由它鉴权);信任域(计划书 5.2)与 **ADR-7 投影脱敏边界零改动**。
    - **仍未定(实施前须钉死)**:票据形态(URL query 还是 fragment;长度 / 熵 / 有效期 / 单次消费与重放防护);**签发端点**的路由、鉴权、限流键与错误语义(**属新契约面,须先契约后实现**);换票响应形态与失败语义(过期 / 已消费 / 租户不匹配);票据**进访问日志 / Referer 泄漏**的处置;**产品定位表述与 `docs/user/**` 回填时机**。
    - **安全论证要点**:租户 / 题目由**服务端签发票据**绑定,URL 仅导航 ⇒ **须机检「URL 参数不得进入租户派生路径」**;票据为**一次性短时**凭证,页面不做授权判断。
    - **派生义务更新**:新增**「启动票据 / 启动地址」契约面**(走契约纪律 5.6);计划书与四条底线第 4 条的修订**已于 2026-09-18 执行完毕(文档面)**(依据 = §五 已于同日成文);**当前阻塞 = 实现未开工**。

### D-API-154 启动票据 / 启动地址契约(页面分发的授权入口;需求层定案,**实现未开工**)

- **决策**(六项裁定,**全部采纳草案建议**;权威文本 = `docs/develop/启动票据契约设计草案.md` **§〇.1**):
  1. **D1 票据在 URL 中的承载方式 = query + 服务端立即消费 + 302 抹除** —— **零 JS 依赖**(换票全程在服务端完成,票据不在不可信侧被处理);`GET` 时原子消费 → `Set-Cookie` → **302 到不含票据的干净路径**。**代价如实登记**:票据**会进服务端访问日志** ⇒ 附带**强制**「应用侧日志脱敏机检」+ **部署指南把反代侧脱敏列为运维硬要求**(反代不在本项目控制内,D-API-104 定案为反代准入)。
  2. **有效期缺省 300 s**(配置键 `SESSION_API_LAUNCH_TICKET_TTL_SECONDS`,天花板 3600);熵 **≥ 128 bit CSPRNG**。
  3. **签发端点 = `POST /auth/launch-tickets`**(承继 `EMBED_TOKEN_ISSUANCE_ROUTE = "/auth/embed-tokens"` 的位置与鉴权姿态;后者**退役**);鉴权**复用 `hostBackendTokenMatches`(单份实现,不得复制)**;请求体**恰两键** `{challengeId, version}`(公开导航信息);响应**恰两键** `{launchUrl, expiresAt}`;限流新增 `RateLimitDimension` 值 `launch_ticket_rate`,键 `rate:{锚租户}:launch_tickets`(锚户 = 绑定集合字典序最小项,沿 D-API-125),**缺省 60 次/分**、天花板 100000、触顶 **429 逐字节沿冻结常量**。
  4. **会话固定缓解 = 基线**(票据**一次性** + TTL 300 s + 平台侧「地址只发给对应学习者」交付纪律 + **`Sec-Fetch-Mode: navigate` 校验** —— 纯请求头,拒绝子资源 / 嵌入式换票)。**未采纳**「换票改 POST + 同站校验」:它与第 1 项的**零 JS** 在结构上不相容,**二者必须一致取舍**;残留风险登记为**中低**(教学场景可接受),若判定不可接受**须整体改走 fragment + JS 换票**。
  5. **`/auth/embed-tokens` 与嵌入协议同批硬切,不留过渡别名**(与「不留并存窗口」的既有定案一致)。
  6. **不绑定 IP / UA** —— 绑定会误伤移动端与共享出口,并引入「绑定不匹配」这一**可探测的差异面**(与「失败统一 401 形态」冲突)。
- **复用的既有先例(不另造轮子)**:① 单次消费令牌 `token:{jti}`(**fail-closed、不可降级**,计划书 `:427`);② `hostBackendTokenMatches` 单份实现(`auth/plugin.ts:111`);③ `FixedWindowRateGate` + `rate:{锚租户}:<维度>` + 429 冻结形态(D-API-50 / D-API-125);④ 凭证 × `SESSION_API_HOST_TENANTS` 白名单(**参数不得派生租户**,O-MP-6);⑤ 「过期 / 已消费 / 绑定不符 / 未认证 ⇒ **全部 401 统一形态**」(阶段三 `:483`)。
- **与 `:341` 旧纪律的关系(须正面回答)**:计划书 §8.1 曾立「**不应把长期凭证放入 iframe URL**」。**启动票据不是长期凭证**(一次性 + 300 s),但**它仍进 URL** ⇒ 本决策的**核心即关上该暴露窗口**(302 抹除 + 首屏无子资源 + 日志脱敏 + `Sec-Fetch` 校验),而非以「它很短」绕过该纪律。
- **存储与可用性**:Redis 新键域 **`launch:{jti}`**(带 TTL);**fail-closed** —— Redis 不可用 ⇒ 换票 **503 `store_unavailable`**,**不降级、不静默放行**(与 `token:{jti}` / `route:{sessionId}` 同纪律);Redis 掉电 ⇒ 未消费票据失效(优雅失败:平台**可无限重签**);票据**绝不进 PG / 审计账 / 指标标签**。**新键域须同批登记**(计划书 §5.7 与本台账键域表)。
- **红线对照**:租户 / 题目由**服务端签发的票据**绑定、URL 仅导航 ⇒ **须机检「URL 与 body 参数不得进入租户派生路径」**;ADR-7 投影脱敏与信任域**零改动**;`/host/scores`(WP-78)、`SESSION_API_HOST_BACKEND_TOKEN`、`SESSION_API_HOST_TENANTS`、会话凭证 / Cookie / WSS 认证 preHandler **全部保留不动**。
- **实现状态:未开工(须显式声明)** —— 本决策为**需求层**;`packages/protocol` 的契约族与 `LAUNCH_TICKET_PROTOCOL_VERSION` **尚未建立** ⇒ 按契约纪律 5.6 **须先契约后实现**(改分类论证 → 改契约包与 fixture → 评审 → 再改实现);`fixtures:manifest` 重算与 `smoke:contract` 随契约建立同批。
- **实施前须补(草案 §十二 登记)**:契约包 Zod schema 与 JSON Schema 产物;平台侧迁移指引(`docs/user/宿主平台接入指南.md` 从「换 embed token + 嵌 iframe」改为「调签发端点 + 下发地址」);`page-app` 托管形态与构建配置;运维面 TLS / 域名 / 反代路径 / **日志脱敏具体配置**(**未实测不给配置片段**,D-API-138 纪律);既有嵌入面 E2E 的退役处置。
