...
- **门禁形态自查**:`e2e-matrix` **不进 turbo 任务图**(保持 CI 承载;本地复跑命令与 env 清单在 `apps/plugin-dev/README.md`)。**E-3 未达成**:job 已落地并通过静态校验,但「在 CI 实跑绿至少一次」须在推送后由 ubuntu 产出(本机 webkit 为**环境级阻断**:Windows headless 建不了认证 WSS 动作通道 ⇒ `[client_error] 动作通道未连接`,非产品缺陷;这正是该义务被限定为「WebKit **linux** 复跑」的原因)。
