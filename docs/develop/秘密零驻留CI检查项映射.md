...
| `compose-integration` | 安装依赖 → 构建 TS 工作区 → ZR-B9 / B10 / B5 / B4 扫描器规则与红灯反例 + ZR-B12 / B13 单测面(vitest `test/scan`;WP-44 起)→ **完整 linux 拓扑**构建与启动(PostgreSQL + Redis + MinIO + session-api + vm-worker;`WORKER_CARGO_PROFILE=release`,D-API-64)→ Compose 集成套件(全链路 + 跨域载荷机检零命中 + 编排器重启恢复;D-API-65)→ 拓扑收尾(down -v) | 质量门禁 7(阶段三子集)、ZR-B9 / B10 / B5 / B4 / B1 / B6(通道录制面)、ZR-B12 / B13(单测面;集成层 SESSION_API_IT 门控)、ZR-T1~T4 |
...
| ZR-B7 | 内部堆栈与文件路径 | **服务器侧日志面(WP-1,2026-09-09)**:`apps/session-api` 日志纪律——err 序列化器只保留 type + message(堆栈 / 文件路径不入日志;`SESSION_API_LOG_ERROR_STACKS` 受控排障显式开关,默认关闭)、req 序列化器白名单(method / url 之外结构性不入日志)、pino redact 路径表兜底 seed / flag / 凭证浅层误放;错误响应面 = 冻结 `PublicError` 形态零细节透出(细节只进受控日志,权威 API 语义规约 D-API-9)。**浏览器响应面(WP-55,2026-09-11)嵌入面收口**:①嵌入协议 V-12 失败静默——一切校验违规丢弃 + 两侧本地计数(embed-runtime `ViolationCounters` / 插件 `violationCounters` 诊断 getter),零对端反馈、零中断,E2E 逐规则断言(宿主窗口不受信来源 → `v1-origin-mismatch`、重复 hello → `state-hello-after-ready`、过期 seq → `v7-stale-seq`、方向违规 → `v6-wrong-direction`;插件侧错 esid → `v5-session-mismatch`、方向 → `v6`、过期 seq → `v7`),落点 `apps/plugin-dev/e2e/embed-protocol.spec.ts`「伪造消息与不受信来源」用例(诊断注入面板 = 宿主模拟页联调面);②受控日志零内部细节——四个浏览器可达包(embed-runtime / react-wrapper / web-component / vm-ui)源码**零 console 调用**(结构前提),E2E 注入窗口 console error/warning 抽样断言为零,降级面 `pwn-degraded` 纯静态文案零反射(不携带 esid / 端点 / 消息细节,同 spec 内断言);③构建面既有 `scan:public` 五包零违规继续覆盖浏览器产物 | ✅(服务器侧日志面 ✅ WP-1 / 浏览器响应面 ✅ WP-55 嵌入面收口) | 红灯反例:`apps/session-api/test/logging.test.ts`(语料顶层与嵌套遮蔽、堆栈零外泄、头部不入日志)+ `test/server.test.ts`(响应面零内部细节、解析器细节只进日志);嵌入面注入反例 = `embed-protocol.spec.ts` 伪造消息用例(每条注入违规必在对应计数键递增,反例与断言同套件) |
...
| ZR-B12 | 调试通道出站帧承载变体外内存字节(调试通道帧语料 ⊆ 调试变体镜像;公开性由零装载构造保证,ADR-DC1 条款 2;清单 §9.1 v1.13 增补) | 扫描器 `apps/session-api/src/scan/debug-frame-corpus-scanner.ts`(纯模块零依赖):S→C 帧内存字节字段(`debug_window_data.bytesHex` / `debug_search_results.hits[].bytesHex` / `debug_instruction_stream.instructions[].bytesHex`,断言面封闭枚举导出 `DEBUG_FRAME_MEMORY_BYTE_POSITIONS`)逐值断言 ⊆ 变体自身 `memoryRegions.contentHex`(按帧 `addressHex` 定位区域后子串包含)+ 玩家输入回显语料(§6.8 封闭集第三源);地址类字段与展示文本字段(`instructions[].text` / `functions[].label`)按 Schema 位置白名单放行(`DEFAULT_DEBUG_TEXT_ALLOWLIST`,白名单是输入面可测试收紧),放行文本附走私字节边界检查(> 16 十六进制字符的语料外串必命中)。双层运行:单测 `test/scan/debug-frame-corpus-scanner.test.ts`(常规 CI;compose-integration job `vitest run test/scan` 同步承载)+ 集成 `test/scan/debug-frame-corpus.integration.test.ts`(SESSION_API_IT 门控;**生产变体 + 真实 worker**:`productionDebugVariantProvider` 包装捕获编排器实际装载变体,录制 attach→window→step→search→instruction_stream→function_table 全程出站帧零命中) | ✅(单测 ✅ 常规 CI / 集成 ✅ SESSION_API_IT 门控) | 红灯反例(与零命中同文件同套件):①注入帧字节 = SECRET_FLAG_PLACEHOLDER / SEED_HEX_FIXTURE 占位秘密语料 → 必命中;②注入帧字节 = 变体外合成随机字节 → 必命中;③越界地址帧 → `ZR-B12-address-outside-variant` 必命中;④文本走私语料外超长十六进制串 → `ZR-B12-text-smuggled-bytes` 必命中;白名单边界与批量报告反例在单测(`debug-frame-corpus-scanner.test.ts`) |
| ZR-B13 | 调试变体派生声明不可复核(变体秘密槽字节与声明 `algorithmId` / 规范槽序 / 调试种子不一致;清单 §9.1 v1.13 增补) | 复算检查器 `apps/session-api/src/scan/debug-variant-derivation-checker.ts`(仅依赖 `@stackmaster/challenge-compiler` 派生原语——SeedDeriver / deriveAslrBaseAddress / 算法标识,session-api 生产 Provider 同源依赖,无新增依赖边):按 WP-40 §七.2 规范槽序复算基址槽 / 隐藏区域槽 / canary 槽字节,与变体 `contentHex` / `derivation.baseAddresses` / `derivation.draws` 逐项比对,命中可归因到派生面(阶段 A 隐藏区域 / 阶段 B canary 叠加)。单测 `test/scan/debug-variant-derivation-checker.test.ts`(buildDebugVariantBundle 产出 × 同种子复算零命中,ASLR 开 / 关双形态,draws 记账 514/513 一致)+ 集成 `test/scan/debug-frame-corpus.integration.test.ts`(生产 Provider 捕获变体 × 实际种子复算零命中) | ✅(单测 ✅ 常规 CI / 集成零命中面 ✅ SESSION_API_IT 门控;集成层错误种子红灯不可构造——现网字节 fixture 无隐藏区域 / canary ⇒ draws = 0,复算为声明面平凡一致性,如实登记;派生槽非平凡复算随含派生槽题目 fixture 接入) | 红灯反例(单测,与零命中同套件):①错误种子复算 → 基址 / 隐藏区域 / canary 内容必命中(draws 为槽序结构量不误报);②变体隐藏区域单字节篡改 → `ZR-B13-hidden-region-content-mismatch`;③变体 canary 槽字节篡改 → `ZR-B13-canary-content-mismatch`;④算法标识改写 / ASLR 关携带 baseAddresses / canary 槽越界 → 各自必命中 |
...
| ZR-T1 ~ T4(篡改、幂等、伪造) | **编排器侧篡改矩阵(WP-7,2026-09-10)**:`apps/session-api/test/wp7/zr-tamper-matrix.test.ts`(D-API-62)——重放旧请求(窗口内外均确定性:缓存同形 / conflict;服务端权威锚定,零状态腐化)、伪造成功标志 / 自制快照 / 状态哈希(strictObject 拒绝 + 零校验器细节 + 权威状态零影响)、伪造投影(通道无客户端投影入口;sync-projection 只回 worker 权威缓存投影)、幂等键同键异负载冲突。Compose 拓扑级全链路见 `test/compose`。**ZR-T4 verifier 级复锚(阶段六 WP-63,2026-09-12)**:`apps/session-api/test/compose/compose-full-chain.integration.test.ts`「ZR-T4 verifier 级复锚:伪造 won 成功标志,verifier 独立重放裁决与未伪造会话一致」——同日志双提交行(诚实行 × 伪造 `public_status='won'` 行,同 reference 同 `log_digest`)独立重放裁决逐字面一致 + verdicts 单行幂等;verifier 管线对 public_status 零读取(伪造成功标志不改成绩结构性成立,D-API-100;嵌入协议内伪造成功标志无承载通道,V-9 信封冻结) | ✅(WP-7,2026-09-10;**verifier 级复锚 ✅ WP-63,2026-09-12**) | 红灯反例:矩阵内每类伪造注入必须错误帧 / 确定性拒绝;机检零命中与反例同套件运行;ZR-T4 复锚断言 = 伪造行 verdict === 诚实行 verdict(容忍矩阵:任一不一致即红灯) |
