...
| 版本 | v1.1 |
| 日期 | 2026-09-04 |
| 状态 | 阶段一 WP-4 交付物,指令面 / 谓词面 / 编排面词汇冻结;v1.1 按《Vm 模块设计冲突与整改方案》完成 G1/G3 重定基 |
| 修订 | v1.1(2026-09-04):G1——字长改为题目 `vmProfile.archBits` 位宽声明(32/64),新增操作数宽度域(立即数无符号、位移有符号,按 archBits 掩蔽,6.2);G3——内存区域增补作者自定义 `custom` 类(五类扩为六类),区域大小与页大小收紧为 4KB 的倍数(VMA 页对齐,D2);指令面 21 opcode 的重定基归 G4(另行批次);v1(2026-09-03):冻结初版 |
...
| 上游依据 | 计划书 6.1(MVP 虚拟硬件)、6.2(archBits 位宽掩蔽域)、7.2(可扩展内容与扩展语义)、7.3(DSL 安全边界)、7.4(版本管理);WP-1 清单 **v1.4** §3.2(寄存器冻结基集)、§12(双包字段级分类);`docs/develop/Vm 模块设计冲突与整改方案.md`(G1/G3 裁决) |
...
- **操作数宽度域**(G1):指令立即数为**无符号**架构值,取值 `[0, 2^archBits − 1]`;内存位移为**有符号**架构值,取值 `[−2^(archBits−1), 2^(archBits−1) − 1]`;越界由检查器规则 `XS-ARCH-WIDTH` 拒绝(WP-1 v1.4 §12.6)。§三各 opcode 语义摘要中的"64 位"字样,在 G4 重定基前一律按"archBits 位宽掩蔽域"读;
...
操作数类别:`R` = 寄存器(基集 ∪ FLAG 子集);`I` = 立即数(十六进制,无符号,archBits 位宽域,§二);`M` = 内存操作数(base 寄存器 + 有符号位移,archBits 位宽域,§二)。每指令操作数 ≤ 4。
