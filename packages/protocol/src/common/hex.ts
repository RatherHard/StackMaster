/**
 * 十六进制标量 Schema(计划书 6.2:地址与架构值在公开 API 用明确的十六进制字符串)。
 *
 * 输入校验刻意宽松(1–16 位、大小写均可):规范化形态(位宽补齐、大小写)
 * 由 WP-6 的规范化序列化规则冻结;是否匹配题目位宽(G1/D1:公开包
 * `vmProfile.archBits` 声明 32/64,架构值以 64 位容器承载、高位按位宽掩蔽)
 * 由服务端按题目重新校验——动作协议与 VM Profile 解耦。
 */
import { z } from "zod";
import { MAX_WRITE_BYTES } from "./limits.js";

/** 地址:0x 前缀,1–16 位十六进制数字(≤ 64 位容器)。 */
export const AddressHexSchema = z
  .string()
  .regex(/^0x[0-9a-fA-F]{1,16}$/, "必须是 0x 前缀的十六进制地址(1-16 位数字)");

/** 架构值(archBits 位宽,64 位容器承载):0x 前缀,1–16 位十六进制数字(push 的操作数等)。 */
