...
export type PayloadStep =
  ...
  | {
      ...
      readonly blockId: string | null;
      /**
       * 断点积木的**编译期可解析目标地址**(WP-76;`0x` + 小写)。
       *
       * **非协议面登记**:本字段是浏览器侧编译产物(`PayloadProgram`)的内部结构,
       * 不是 `@stackmaster/protocol` 冻结契约的一部分——不进动作协议、不进
       * 调试通道帧、不进任何网络或持久化面,故为 additive 演进,零契约版本成本。
       *
       * 解析语义(compile.ts 实现,边界见该处注释):断点积木**没有地址输入**
       * (积木定义面不在本包),地址只能来自**编译期公开投影只读快照**的当前
       * 指令指针(寄存器 `rip`)。客户端在编译期不可能知道"未来某步之后 rip
       * 落在何处",因此本地推导未来地址(例如按动作语义推演栈帧 / 返回地址)是
       * **禁止的隐藏信息推断**,本字段只承载已下发投影里**当下**可见的位置。
       *
       * 缺席 = 编译期不可解析(未接数据源 / 投影白名单无 `rip`):保持无地址
       * 形态——解题档步进暂停语义不受影响,调试档并入断点集时该步骤**不产出**
       * 地址(不伪造、不猜测)。
       */
